...
// Simple AI moderation: blocks posts containing banned words
...
export const handler = async (event) => {
  ...
  if (event.httpMethod === "POST") {
    ...
    if (contentType && contentType.startsWith("multipart/form-data")) {
      ...
      if (fileData) {
        const { data, error } = await supabase.storage
          .from("uploads")
          .upload(`posts/${Date.now()}_${fileData.filename}`, fileData.buffer, { contentType: fileData.mime });
    ...
    // Handle JSON actions: signup, login, appeal
    const body = JSON.parse(event.body);

    // SIGNUP
    if (body.action === "signup") {
      const { data, error } = await supabase.from("users").insert({ username: body.u, password: body.p });
      return { statusCode: 200, body: JSON.stringify({ ok: !error }) };
    }
